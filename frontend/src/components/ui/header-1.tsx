	...
	return (
		...
		>
			<nav className="mx-auto flex h-14 w-full max-w-5xl items-center justify-between px-4">
				<div className="hover:bg-accent rounded-md p-2 cursor-pointer">
				...
				<div className="hidden items-center gap-2 md:flex">
					...
					{isLoggedIn ? (
						<>
							<Button variant="outline" onClick={onDashboardClick}>Dashboard</Button>
			...
			<MobileMenu open={open} className="flex flex-col justify-between gap-2">
				...
				<div className="flex flex-col gap-2">
					{isLoggedIn ? (
						<>
							<Button variant="outline" className="w-full bg-transparent" onClick={() => { onDashboardClick?.(); setOpen(false); }}>
								Dashboard
							</Button>
							<Button className="w-full" onClick={() => { onLogoutClick?.(); setOpen(false); }}>Logout</Button>
						</>
					) : (
						<>
							<Button variant="outline" className="w-full bg-transparent" onClick={() => { onLoginClick?.(); setOpen(false); }}>
								Sign In
							</Button>
							<Button className="w-full" onClick={() => { onSignUpClick?.(); setOpen(false); }}>Get Started</Button>
						</>
					)}
				</div>
			</MobileMenu>
